// =====================================================
// EKAIDAN — site_js_final.js  (v4 — all fixes applied)
// =====================================================
...
const AUTH = {
  ...
  async signUp(email, password, options = {}) {
    ...
    let url = `${this._base}/signup`;
    if (options.emailRedirectTo)
      url += `?redirect_to=${encodeURIComponent(options.emailRedirectTo)}`;
    const r = await fetch(url, {
      method: 'POST',
      headers: { apikey: SUPABASE_ANON, 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    });
    const data = await r.json().catch(() => ({ error: { message: 'Unexpected signup response.' } }));
    if (!r.ok) return { error: { message: data.error?.message || data.msg || 'Signup failed.' } };
    return data;
  },

  async resendConfirmation(email, redirectTo) {
    const body = { type: 'signup', email };
    if (redirectTo) body.redirect_to = redirectTo;
    const r = await fetch(`${this._base}/resend`, {
      method: 'POST',
      headers: { apikey: SUPABASE_ANON, 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    });
    const data = await r.json().catch(() => ({ error: { message: 'Unexpected resend response.' } }));
    if (!r.ok) return { error: { message: data.error?.message || data.msg || 'Resend failed.' } };
    return data;
  },
  ...
  async signIn(email, password) {
    ...
    const data = await r.json().catch(() => ({ error: { message: 'Unexpected login response.' } }));
    if (!r.ok) return { error: { message: data.error?.message || data.msg || 'Sign in failed.' } };
    return data;
  },
